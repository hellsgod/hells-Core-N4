/*
 * drivers/cpufreq/cpufreq_proactive.c
 *
 ...
 * Author: jamevu (jamevu@gmail.com) Minor Modifications for proactive
 */
 ...
 */
#define DEFAULT_MIN_SAMPLE_TIME (140 * USEC_PER_MSEC)
...
static int cpufreq_governor_proactive(struct cpufreq_policy *policy,
		unsigned int event)
{
	int rc;
	unsigned int j;
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_PROACTIVE
static
#endif
struct cpufreq_governor cpufreq_gov_proactive = {
	.name = "proactive",
	.governor = cpufreq_governor_proactive,
...
static int __init cpufreq_proactive_init(void)
{
	unsigned int i, rc;
	struct cpufreq_interactive_cpuinfo *pcpu;
	...
	return cpufreq_register_governor(&cpufreq_gov_proactive);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_PROACTIVE
fs_initcall(cpufreq_proactive_init);
#else
module_init(cpufreq_proactive_init);
#endif
...
static void __exit cpufreq_interactive_exit(void)
{
	...
	cpufreq_unregister_governor(&cpufreq_gov_proactive);
...
MODULE_DESCRIPTION("'cpufreq_proactive' - A cpufreq governor for "
