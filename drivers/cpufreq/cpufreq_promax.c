/*
 * drivers/cpufreq/cpufreq_promax.c
 *
...
#ifdef CONFIG_CPU_FREQ_GOV_PROMAX_TEGRA
...
#ifdef CONFIG_CPU_FREQ_GOV_PROMAX_ENRC2B
...
#ifdef CONFIG_CPU_FREQ_GOV_PROMAX_PRIMOU
...
#ifdef CONFIG_CPU_FREQ_GOV_PROMAX_M7
...
#ifdef CONFIG_CPU_FREQ_GOV_PROMAX_FIND5
...
#define CONFIG_CPU_FREQ_GOV_PROMAX_MAKO
#ifdef CONFIG_CPU_FREQ_GOV_PROMAX_MAKO
#define DEFAULT_SUSPEND_IDEAL_FREQ 189000
...
static DEFINE_PER_CPU(struct promax_info_s, promax_info);

#define dprintk(flag,msg...) do { \
	if (debug_mask & flag) pr_info("[promax]" ":" msg); \
	} while (0)

enum {
	PROMAX_DEBUG_JUMPS = 1,
	PROMAX_DEBUG_LOAD = 2,
	PROMAX_DEBUG_ALG = 4,
	PROMAX_DEBUG_BOOST = 8,
	PROMAX_DEBUG_INPUT = 16,
	PROMAX_DEBUG_SUSPEND = 32
};

/*
 * Combination of the above debug flags.
 */
//static unsigned long debug_mask = PROMAX_DEBUG_LOAD|PROMAX_DEBUG_JUMPS|PROMAX_DEBUG_ALG|PROMAX_DEBUG_BOOST|PROMAX_DEBUG_INPUT|PROMAX_DEBUG_SUSPEND;
static unsigned long debug_mask;

#define PROMAX_STAT 0
#if PROMAX_STAT
...
static struct workqueue_struct *promax_wq;
...
static int cpufreq_governor_promax(struct cpufreq_policy *policy,
		unsigned int event);

#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_PROMAX
static
#endif
struct cpufreq_governor cpufreq_gov_promax = { 
    .name = "promax", 
    .governor = cpufreq_governor_promax, 
...
inline static void promax_update_min_max(
		struct promax_info_s *this_promax, struct cpufreq_policy *policy) {
	this_promax->ideal_speed = // ideal_freq; but make sure it obeys the policy min/max
...
inline static void promax_update_min_max_allcpus(void) {
	unsigned int cpu;

	for_each_online_cpu(cpu)
	{
		struct promax_info_s *this_promax = &per_cpu(promax_info, cpu);
		if (this_promax->cur_policy){
			if (lock_policy_rwsem_write(cpu) < 0)
				continue;

			promax_update_min_max(this_promax, this_promax->cur_policy);
...
static inline void dbs_timer_init(struct promax_info_s *this_promax) {
	int delay = get_timer_delay();

	INIT_DELAYED_WORK_DEFERRABLE(&this_promax->work, do_dbs_timer);
	schedule_delayed_work_on(this_promax->cpu, &this_promax->work, delay);
}

static inline void dbs_timer_exit(struct promax_info_s *this_promax) {
	cancel_delayed_work_sync(&this_promax->work);
}

inline static void target_freq(struct cpufreq_policy *policy,
		struct promax_info_s *this_promax, int new_freq, int old_freq,
		int prefered_relation) {
	int index, target;
	struct cpufreq_frequency_table *table = this_promax->freq_table;
	unsigned int cpu = this_promax->cpu;

	dprintk(PROMAX_DEBUG_ALG, "%d: %s\n", old_freq, __func__);
	...
	if (!cpufreq_frequency_table_target(policy, table, new_freq,
		...
		dprintk(PROMAX_DEBUG_ALG, "frequency change failed\n");
		return;
	}

	dprintk(PROMAX_DEBUG_JUMPS, "%d: jumping to %d (%d) cpu %d\n", old_freq, new_freq, target, cpu);

	__cpufreq_driver_target(policy, target, prefered_relation);

	// remember last time we changed frequency
	this_promax->freq_change_time = ktime_to_us(ktime_get());
}

/* We use the same work function to sale up and down */
static void cpufreq_promax_freq_change(struct promax_info_s *this_promax) {
	unsigned int cpu;
	...
	ramp_dir = this_promax->ramp_dir;
	old_freq = this_promax->old_freq;
	policy = this_promax->cur_policy;
	cpu = this_promax->cpu;
	...
	if (old_freq != policy->cur) {
		...
		dprintk(PROMAX_DEBUG_ALG, "%d: frequency changed by 3rd party to %d\n",
				old_freq, policy->cur);
		new_freq = old_freq;
	} else if (ramp_dir > 0 && nr_running() > 1) {
		// ramp up logic:
		if (old_freq < this_promax->ideal_speed)
			new_freq = this_promax->ideal_speed;
		...
		if (old_freq > this_promax->ideal_speed) {
			...
			new_freq = old_freq * this_promax->cur_cpu_load / max_cpu_load;
	...
	if (new_freq!=0){
		target_freq(policy, this_promax, new_freq, old_freq, relation);
	}
	
	this_promax->ramp_dir = 0;
}

static inline void cpufreq_promax_get_ramp_direction(struct promax_info_s *this_promax, u64 now)
{
	unsigned int cur_load = this_promax->cur_cpu_load;
	unsigned int cur = this_promax->old_freq;
	struct cpufreq_policy *policy = this_promax->cur_policy;
	
	// Scale up if load is above max or if there where no idle cycles since coming out of idle,
	// additionally, if we are at or above the ideal_speed, verify we have been at this frequency
	// for at least up_rate:
	if (cur_load > max_cpu_load && cur < policy->max
			&& (cur < this_promax->ideal_speed
				|| (now - this_promax->freq_change_time) >= up_rate)) {
		dprintk(PROMAX_DEBUG_ALG,
				"%d: ramp up: load %d\n", cur, cur_load);
		this_promax->ramp_dir = 1;
	}
	...
	// frequency we require that we have been at this frequency for at least down_rate:
			...
			&& (cur > this_promax->ideal_speed
				|| (now - this_promax->freq_change_time) >= down_rate)) {
		dprintk(PROMAX_DEBUG_ALG,
				"%d: ramp down: load %d\n", cur, cur_load);
		this_promax->ramp_dir = -1;
	}
...
static void cpufreq_promax_timer(struct promax_info_s *this_promax) {
	unsigned int cur;
	struct cpufreq_policy *policy = this_promax->cur_policy;
	...
	timer_stat[cpu] = now;
	printk(KERN_DEBUG "[promax]:cpu %d %lld\n", cpu, diff);
#endif

	cur = policy->cur;

	dprintk(PROMAX_DEBUG_ALG, "%d: %s cpu %d %lld\n", cur, __func__, cpu, now);

	for_each_cpu(j, policy->cpus)
	{
		struct promax_info_s *j_this_promax;
		u64 cur_wall_time, cur_idle_time, cur_iowait_time;
		unsigned int idle_time, wall_time, iowait_time;
		unsigned int cur_load;
		
		j_this_promax = &per_cpu(promax_info, j);

		cur_idle_time = get_cpu_idle_time(j, &cur_wall_time, io_is_busy);
		cur_iowait_time = get_cpu_iowait_time(j, &cur_wall_time);

		wall_time = cur_wall_time - j_this_promax->prev_cpu_wall;
		j_this_promax->prev_cpu_wall = cur_wall_time;

		idle_time = cur_idle_time - j_this_promax->prev_cpu_idle;
		j_this_promax->prev_cpu_idle = cur_idle_time;

		iowait_time = cur_iowait_time - j_this_promax->prev_cpu_iowait;
		j_this_promax->prev_cpu_iowait = cur_iowait_time;

		if (ignore_nice) {
			u64 cur_nice;
			unsigned long cur_nice_jiffies;

#ifdef CONFIG_CPU_FREQ_GOV_PROMAX_30
			cur_nice = kstat_cpu(j).cpustat.nice - j_this_promax->prev_cpu_nice;
			cur_nice_jiffies = (unsigned long) cputime64_to_jiffies64(cur_nice);

			j_this_promax->prev_cpu_nice = kstat_cpu(j).cpustat.nice;
#else
			cur_nice = kcpustat_cpu(j).cpustat[CPUTIME_NICE] - j_this_promax->prev_cpu_nice;
			cur_nice_jiffies = (unsigned long) cputime64_to_jiffies64(cur_nice);

			j_this_promax->prev_cpu_nice = kcpustat_cpu(j).cpustat[CPUTIME_NICE];
		...
		j_this_promax->cur_cpu_load = cur_load;
	}

	/* calculate the scaled load across CPU */
	load_at_max_freq = (this_promax->cur_cpu_load * policy->cur)/policy->cpuinfo.max_freq;

	cpufreq_notify_utilization(policy, load_at_max_freq);

	dprintk(PROMAX_DEBUG_LOAD, "%d: load %d\n", cpu, this_promax->cur_cpu_load);

	this_promax->old_freq = cur;
	this_promax->ramp_dir = 0;

	cpufreq_promax_get_ramp_direction(this_promax, now);

	// no changes
	if (this_promax->ramp_dir == 0)		
		return;

	// boost - but not block ramp up steps based on load if requested
	if (boost_running){
		if (now < boost_end_time) {
			dprintk(PROMAX_DEBUG_BOOST, "%d: cpu %d boost running %llu %llu\n", cur, cpu, now, boost_end_time);
		
			if (this_promax->ramp_dir == -1)
				return;
			else {
				if (ramp_up_during_boost)
					dprintk(PROMAX_DEBUG_BOOST, "%d: cpu %d boost running but ramp_up above boost freq requested\n", cur, cpu);
				else
					return;
			}
		} else
	...
	cpufreq_promax_freq_change(this_promax);
}

static void do_dbs_timer(struct work_struct *work) {
	struct promax_info_s *this_promax =
			container_of(work, struct promax_info_s, work.work);
	unsigned int cpu = this_promax->cpu;
	int delay = get_timer_delay();

	mutex_lock(&this_promax->timer_mutex);

	cpufreq_promax_timer(this_promax);

	queue_delayed_work_on(cpu, promax_wq, &this_promax->work, delay);
	mutex_unlock(&this_promax->timer_mutex);
}
...
static void update_idle_time(bool online) {
	...
	for_each_possible_cpu(j)
	{
		...
		j_this_promax->prev_cpu_idle = get_cpu_idle_time(j,
				&j_this_promax->prev_cpu_wall, io_is_busy);
...
static struct attribute * promax_attributes[] = { 
...
static struct attribute_group promax_attr_group = { 
	.attrs = promax_attributes, 
	.name = "promax", 
	};

static int cpufreq_promax_boost_task(void *data) {
	struct promax_info_s *this_promax;
	u64 now;
	struct cpufreq_policy *policy;
#ifndef CONFIG_CPU_FREQ_GOV_PROMAX_TEGRA
	unsigned int cpu;
	...
	while (1) {
		...
		this_promax = &per_cpu(promax_info, 0);
		if (!this_promax)
			continue;
        ...
        this_promax->prev_cpu_idle = get_cpu_idle_time(0,
						&this_promax->prev_cpu_wall, io_is_busy);

        unlock_policy_rwsem_write(0);
#else		
		for_each_online_cpu(cpu){
			this_promax = &per_cpu(promax_info, cpu);
			if (!this_promax)
				continue;
			...
			if (policy->cur < cur_boost_freq) {
				start_boost = true;
				dprintk(PROMAX_DEBUG_BOOST, "input boost cpu %d to %d\n", cpu, cur_boost_freq);
				target_freq(policy, this_promax, cur_boost_freq, this_promax->old_freq, CPUFREQ_RELATION_H);
				this_promax->prev_cpu_idle = get_cpu_idle_time(cpu, &this_promax->prev_cpu_wall, io_is_busy);
			}
			mutex_unlock(&this_promax->timer_mutex);
		...
		dprintk(PROMAX_DEBUG_BOOST, "%s %llu %llu\n", __func__, now, boost_end_time);
	...
	pr_info("[promax]:" "%s boost_thread stopped\n", __func__);
	return 0;
}

#ifdef CONFIG_INPUT_MEDIATOR

static void promax_input_event(struct input_handle *handle, unsigned int type,
...
static struct input_mediator_handler promax_input_mediator_handler = {
	.event = promax_input_event,
	};
...
static int dbs_input_connect(struct input_handler *handler,
	...
	pr_info("[promax]:" "%s input connect to %s\n", __func__, dev->name);
	...
	err2: kfree(handle);
	pr_err("[promax]:" "%s faild to connect input handler %d\n", __func__, error);
	return error;
}
...
static int cpufreq_governor_promax(struct cpufreq_policy *new_policy,
		unsigned int event) {
	...
	case CPUFREQ_GOV_START:
		...
		this_promax->cur_policy = new_policy;
		this_promax->cpu = cpu;

		promax_update_min_max(this_promax,new_policy);

		this_promax->freq_table = cpufreq_frequency_get_table(cpu);
		if (!this_promax->freq_table){
		...
		if (dbs_enable == 1) {
			if (!boost_task_alive) {
				boost_task = kthread_create (
						cpufreq_promax_boost_task,
						NULL,
						"promax_input_boost_task"
				);
				...
				pr_info("[promax]:" "%s input boost task created\n", __func__);
				sched_setscheduler_nocheck(boost_task, SCHED_FIFO, &param);
				get_task_struct(boost_task);
				boost_task_alive = true;
			}
#ifdef CONFIG_INPUT_MEDIATOR
			input_register_mediator_secondary(&promax_input_mediator_handler);
#else
	...
	case CPUFREQ_GOV_LIMITS:
		...
		if (this_promax->cur_policy->cur > new_policy->max) {
			dprintk(PROMAX_DEBUG_JUMPS,"CPUFREQ_GOV_LIMITS jumping to new max freq: %d\n",new_policy->max);
			__cpufreq_driver_target(this_promax->cur_policy,
					new_policy->max, CPUFREQ_RELATION_H);
		}
		else if (this_promax->cur_policy->cur < new_policy->min) {
			dprintk(PROMAX_DEBUG_JUMPS,"CPUFREQ_GOV_LIMITS jumping to new min freq: %d\n",new_policy->min);
			__cpufreq_driver_target(this_promax->cur_policy,
	...
	case CPUFREQ_GOV_STOP:
		dbs_timer_exit(this_promax);

		mutex_lock(&dbs_mutex);
		this_promax->cur_policy = NULL;
		dbs_enable--;
		...
		if (!dbs_enable){
			...
			sysfs_remove_group(cpufreq_global_kobject, &promax_attr_group);
#ifdef CONFIG_INPUT_MEDIATOR
			input_unregister_mediator_secondary(&promax_input_mediator_handler);
#else
...
static int __init cpufreq_promax_init(void) {
	unsigned int i;
	struct promax_info_s *this_promax;
	u64 wall;
	u64 idle_time;
	...
	promax_wq = alloc_workqueue("promax_wq", WQ_HIGHPRI, 0);
	if (!promax_wq) {
		printk(KERN_ERR "Failed to create promax_wq workqueue\n");
		return -EFAULT;
	}
	...
	{
		this_promax = &per_cpu(promax_info, i);
		this_promax->cur_policy = NULL;
		this_promax->ramp_dir = 0;
		this_promax->freq_change_time = 0;
		this_promax->cur_cpu_load = 0;
		mutex_init(&this_promax->timer_mutex);
	}

	return cpufreq_register_governor(&cpufreq_gov_promax);
}

#ifdef CONFIG_CPU_FREQ_DEFAULT_GOV_PROMAX
fs_initcall(cpufreq_promax_init);
#else
module_init(cpufreq_promax_init);
#endif

static void __exit cpufreq_promax_exit(void) {
	unsigned int i;
	struct promax_info_s *this_promax;

	cpufreq_unregister_governor(&cpufreq_gov_promax);

	for_each_possible_cpu(i)
	{
		this_promax = &per_cpu(promax_info, i);
		mutex_destroy(&this_promax->timer_mutex);
	}
	destroy_workqueue(promax_wq);
}

module_exit(cpufreq_promax_exit);

MODULE_AUTHOR("maxwen");
MODULE_DESCRIPTION("'cpufreq_promax' - A smart cpufreq governor");
MODULE_LICENSE("GPL");
